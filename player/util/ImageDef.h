...
public:
	static void AllocNativeImage(NativeImage *pImage)
	{
		...
		switch (pImage->format)
		{
			...
			case IMAGE_FORMAT_I420:
			{
				...
				break;
			default:
				//qDebug("NativeImageUtil::AllocNativeImage do not support the format. Format = %d", pImage->format);
				break;
		}
	}
	...
	{
	    //qDebug("NativeImageUtil::CopyNativeImage src[w,h,format]=[%d, %d, %d], dst[w,h,format]=[%d, %d, %d]", pSrcImg->width, pSrcImg->height, pSrcImg->format, pDstImg->width, pDstImg->height, pDstImg->format);
        //qDebug("NativeImageUtil::CopyNativeImage src[line0,line1,line2]=[%d, %d, %d], dst[line0,line1,line2]=[%d, %d, %d]", pSrcImg->pLineSize[0], pSrcImg->pLineSize[1], pSrcImg->pLineSize[2], pDstImg->pLineSize[0], pDstImg->pLineSize[1], pDstImg->pLineSize[2]);
		...
		{
			//qDebug("NativeImageUtil::CopyNativeImage invalid params.");
			return;
		}
		...
		switch (pSrcImg->format)
		{
			...
			default:
			{
				//qDebug("NativeImageUtil::CopyNativeImage do not support the format. Format = %d", pSrcImg->format);
			}
				break;
		}
	...
	static void DumpNativeImage(NativeImage *pSrcImg, const char *pPath, const char *pFileName)
	{
		...
		//qDebug("DumpNativeImage fp=%p, file=%s", fp, imgPath);
		...
		if(fp)
		{
			switch (pSrcImg->format)
			{
				...
				default:
				{
					//qDebug("DumpNativeImage default");
					break;
				}
			}
