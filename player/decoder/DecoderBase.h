...
enum DecoderMsg {
    MSG_DECODER_INIT_ERROR, //解码器初始化失败
    MSG_DECODER_READY,//解码器初始化成功  no
    MSG_DECODER_DONE, //关闭解码器 no 
    MSG_DECODING_TIME, //完成解码时间 no 
    MSG_RENDER_TIME, //完成渲染时间
    MSG_PAUSE_TIME,  //暂停时间 no 
    MSG_PLAY_DONE,  //播放完成
    MSG_EXCEPTION  //A发生异常
};
...
private:
    ...
    void DecodingLoop();
    //更新显示时间戳
    void UpdateTimeStamp(long seekPosition,bool seekSuccess);
    //音视频同步
    long AVSync();
    //解码一个packet编码数据
    int DecodeOnePacket();
