...
void DecoderBase::SeekToPosition(float position) {
    //qDebug("DecoderBase::SeekToPosition position=%f", position);
    std::unique_lock<std::mutex> lock(m_Mutex);
...
int DecoderBase::Init(const char *url, AVMediaType mediaType) {
    //qDebug("DecoderBase::Init url=%s, mediaType=%d", url, mediaType);
    strcpy(m_Url,url);
...
void DecoderBase::UnInit() {
    //qDebug("DecoderBase::UnInit m_MediaType=%d", m_MediaType);
    if(m_Thread) {
        Stop();
        m_Thread->join();
        delete m_Thread;
    ...
    //qDebug("DecoderBase::UnInit end, m_MediaType=%d", m_MediaType);
...
int DecoderBase::InitFFDecoder() {
    ...
    do {
        ...
        if(m_StreamIndex == -1) {
            ("DecoderBase::InitFFDecoder Fail to find stream index.");
            break;
        }
        //5.获取解码器参数
...
void DecoderBase::UnInitDecoder() {
    //qDebug("DecoderBase::UnInitDecoder");
    if(m_Frame != nullptr) {
...
void DecoderBase::DecodingLoop() {
    //qDebug("DecoderBase::DecodingLoop start, m_MediaType=%d", m_MediaType);
    {
        std::unique_lock<std::mutex> lock(m_Mutex);
        m_DecoderState = STATE_DECODING;
    }
    ...
    for(;;) {
        while (m_DecoderState == STATE_PAUSE) {
            std::unique_lock<std::mutex> lock(m_Mutex);
            //qDebug("DecoderBase::DecodingLoop waiting, m_MediaType=%d", m_MediaType);
            m_Cond.wait_for(lock, std::chrono::milliseconds(10));
            m_StartTimeStamp = get_system_current_time() - m_CurTimeStamp;
        }
        ...
        if(m_StartTimeStamp == -1)
            m_StartTimeStamp = get_system_current_time();
        ...
        if(DecodeOnePacket() != 0) {
            ...
            m_DecoderState = STATE_PAUSE;
            if(m_MsgContext && m_MsgCallback){
                m_MsgCallback(m_MsgContext, MSG_PLAY_DONE, 0); //返回播放结束
            }
        }
    }
    //qDebug("DecoderBase::DecodingLoop end");
}

void DecoderBase::UpdateTimeStamp(long seekPosition,bool seekSuccess) {
    //qDebug("DecoderBase::UpdateTimeStamp");
    std::unique_lock<std::mutex> lock(m_Mutex);
...
long DecoderBase::AVSync() {
    //qDebug("DecoderBase::AVSync");
    long curSysTime = get_system_current_time();
    //基于系统时钟计算从开始播放流逝的时间
...
int DecoderBase::DecodeOnePacket() {
    //qDebug("DecoderBase::DecodeOnePacket m_MediaType=%d", m_MediaType);
    long seekPosition;
    {
        std::unique_lock<std::mutex> lock(m_Mutex);
    ...
    if(seekPosition > 0) {
        //seek to frame
        int64_t seek_target = static_cast<int64_t>(seekPosition * 1000);//微秒
        int64_t seek_min = INT64_MIN;
        int64_t seek_max = INT64_MAX;
        int seek_ret = avformat_seek_file(m_AVFormatContext, -1, seek_min, seek_target, seek_max, 0);
        if (seek_ret < 0) {
            seekSuccess = false;
            //qDebug("BaseDecoder::DecodeOneFrame error while seeking m_MediaType=%d", m_MediaType);
        } else {
            if (-1 != m_StreamIndex) {
            ...
            ClearCache();
            seekSuccess = true;
            //qDebug("BaseDecoder::DecodeOneFrame seekFrame pos=%f, m_MediaType=%d", m_SeekPosition, m_MediaType);
        }
    }
...
//            }
            ...
            while (avcodec_receive_frame(m_AVCodecContext, m_Frame) == 0) {
                //更新时间戳
                UpdateTimeStamp(seekPosition,seekSuccess);
                //同步
                AVSync();
                //渲染
                //qDebug("DecoderBase::DecodeOnePacket 000 m_MediaType=%d", m_MediaType);
                OnFrameAvailable(m_Frame);
                if(m_MsgContext && m_MsgCallback && m_MediaType == AVMEDIA_TYPE_AUDIO)
                    m_MsgCallback(m_MsgContext, MSG_RENDER_TIME, m_CurTimeStamp);
                //qDebug("DecoderBase::DecodeOnePacket 0001 m_MediaType=%d", m_MediaType);
                frameCount ++;
            }
            //qDebug("BaseDecoder::DecodeOneFrame frameCount=%d", frameCount);
            //判断一个 packet 是否解码完成
            if(frameCount > 0) {
                result = 0;
                goto __EXIT;
...
void DecoderBase::DoAVDecoding(DecoderBase *decoder) {
    //qDebug("DecoderBase::DoAVDecoding");
    do {
        if(decoder->InitFFDecoder() != 0) {
            break;
        }
        decoder->OnDecoderReady();
        if(decoder->m_MsgContext && decoder->m_MsgCallback && decoder->m_MediaType==AVMEDIA_TYPE_AUDIO)
            decoder->m_MsgCallback(decoder->m_MsgContext, MSG_DECODER_READY, decoder->m_Duration);
