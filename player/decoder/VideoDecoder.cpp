...
void VideoDecoder::OnDecoderDone() {
    //qDebug()<<"VideoDecoder::OnDecoderDone";
...
void VideoDecoder::OnFrameAvailable(AVFrame *frame) {
    //qDebug("VideoDecoder::OnFrameAvailable frame=%p", frame);
    if(m_VideoRender != nullptr && frame != nullptr) {
        NativeImage image;
        //qDebug("VideoDecoder::OnFrameAvailable frame[w,h]=[%d, %d],format=%d,[line0,line1,line2]=[%d, %d, %d]", frame->width, frame->height, GetCodecContext()->pix_fmt, frame->linesize[0], frame->linesize[1],frame->linesize[2]);
        if(m_VideoRender->GetRenderType() == VIDEO_RENDER_ANWINDOW)
        {
            ...
            image.pLineSize[1] = m_RGBAFrame->linesize[1];
            image.pLineSize[2] = m_RGBAFrame->linesize[2];
            image.ppPlane[0] = m_RGBAFrame->data[0];
            image.ppPlane[1] = m_RGBAFrame->data[1];
            image.ppPlane[2] = m_RGBAFrame->data[2];
            //image.pLineSize[0] = m_RGBAFrame->linesize[0];
